...
class TestComputeRagasMetrics:
    ...
    def test_passed_false_when_one_threshold_missed(self):
        scorer = FakeRAGScorer(scores={"faithfulness": 0.60, "answer_relevancy": 0.85, "context_recall": 0.80})
        questions, answers, contexts, ground_truths = _make_batch()
        result = compute_ragas_metrics(questions, answers, contexts, ground_truths, scorer=scorer)
    ...
    def test_scorer_called_once(self):
        scorer = FakeRAGScorer()
        questions, answers, contexts, ground_truths = _make_batch()
        compute_ragas_metrics(questions, answers, contexts, ground_truths, scorer=scorer)
    ...
    def test_scores_are_cast_to_float(self):
        scorer = FakeRAGScorer(scores={"faithfulness": 1, "answer_relevancy": 1, "context_recall": 1})
        questions, answers, contexts, ground_truths = _make_batch()
        result = compute_ragas_metrics(questions, answers, contexts, ground_truths, scorer=scorer)
